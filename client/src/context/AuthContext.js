...
const Auth = ({ children }) => {
    ...
    useEffect(() => {
        ...
        axios
            .get(`${process.env.REACT_APP_API_URL}/user`, { headers: { Authorization: access } })
            .then((res) => {
    ...
    return <AuthContext.Provider value={{ access, authenticated, logout, login }}>{children}</AuthContext.Provider>;
};
