...
const Task = (props) => {
    ...
    const toggleComplete = async () => {
        try {
            ...
            setCompletion(data.completed_at);
        ...
        <div className="bg-white rounded-lg p-2 mt-2 border-gray-200 border text-sm flex items-center justify-between">
            <div>
                <button onClick={toggleComplete}>
                    <svg xmlns="http://www.w3.org/2000/svg" className={`h-7 w-7 fill-current text-green-500 opacity-${completed ? '100' : '25'}`} viewBox="0 0 20 20">
                        <path
                            fillRule="evenodd"
            ...
            <div>
                <h1 className={`outline-none ${completed ? 'line-through' : ''}`}>{title}</h1>
            </div>
            <div>
